...
async function main() {
    ...
    appNFT = await AppNFTContract.attach(AppNFTAddress)
    ...
    RoleControl = await upgrades.deployProxy(
        RoleControlContract,
        [nftToken2.address, 1, deployer.address],
        { initializer: "initialize" }
    )
    ...
    Listener = await upgrades.deployProxy(ListenerContract, [], {
        initializer: "initialize"
    })
    ...
    ContractBasedDeployment = await upgrades.deployProxy(
        ...
        [RoleControl.address, Listener.address],
        { initializer: "initialize" }
    )
